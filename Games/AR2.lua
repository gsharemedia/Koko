...
local Loaded,PromptLib = false,loadstring(game:HttpGet("https://raw.githubusercontent.com/gsharemedia/Roblox/main/Useful/PromptLibrary.lua"))()
if identifyexecutor() ~= "Synapse X" then
    PromptLib("Unsupported executor","Synapse X only for safety measures\nYou are at risk of getting autoban\nAre you sure you want to load Koko?",{
...
local Window = Koko.Utilities.UI:Window({
    ...
    end Koko.Utilities.Misc:SettingsSection(Window,"Period",true)
end Koko.Utilities.Misc:InitAutoLoad(Window)
...
Koko.Utilities.Misc:FixUpValue(Network.Send,function(Old,Self,Name,...) local Args = {...}
...
Koko.Utilities.Misc:NewThreadLoop(0.1,function()
...
Events["Inventory Container Added"] = function(Id,Data,...)
    ...
    if Data.WorldPosition and Length(Data.Occupants) > 0 and not string.find(Data.Type,"Corpse") then
        Koko.Utilities.Drawing:AddObject(Data.Id,CIIC(Data),Data.WorldPosition,
...
Events["Container Changed"] = function(Data,...)
    ...
    Koko.Utilities.Drawing:RemoveObject(Data.Id)
    if Data.WorldPosition and Length(Data.Occupants) > 0 and not string.find(Data.Type,"Corpse") then
        Koko.Utilities.Drawing:AddObject(Data.Id,CIIC(Data),Data.WorldPosition,
        "AR2/ESP/Items","AR2/ESP/Items/Containers",Window.Flags)
...
for Index,Item in pairs(Loot:GetDescendants()) do
    ...
    if Item:IsA("CFrameValue") and ItemData then --print(ItemData.Parent.Name)
        Koko.Utilities.Drawing:AddObject(Item,Item.Name,Item.Value.Position,
...
Loot.DescendantRemoving:Connect(function(Item)
    Koko.Utilities.Drawing:RemoveObject(Item)
end)
Randoms.ChildRemoved:Connect(function(Event)
    Koko.Utilities.Drawing:RemoveObject(Event)
end)
Vehicles.ChildRemoved:Connect(function(Vehicle)
    Koko.Utilities.Drawing:RemoveObject(Vehicle)
end)
Zombies.Mobs.ChildRemoved:Connect(function(Zombie)
    Koko.Utilities.Drawing:RemoveObject(Zombie)
end)
