...
local Loaded1,Loaded2,PromptLib = false,false,loadstring(game:HttpGet("https://raw.githubusercontent.com/gsharemedia/Roblox/main/Useful/PromptLibrary.lua"))()
...
Notify.Event:Connect(function(Text)
    Koko.Utilities.UI:Notification2(Text)
end)
...
Koko.Utilities.Misc:FixUpValue(Tortoiseshell.Network.Fire,function(Old,Self,...)
    local Args = {...}
...
Koko.Utilities.Misc:FixUpValue(Tortoiseshell.Projectiles.InitProjectile,function(Old,Self,...)
...
Koko.Utilities.Misc:FixUpValue(Tortoiseshell.Raycast.CastGeometryAndEnemies,function(Old,Self,...)
...
Koko.Utilities.Misc:FixUpValue(Tortoiseshell.Items.GetAnimator,function(Old,Self,...)
    local Args = {...} if Args[1] then WeaponModel = Args[3] end
    return Old(Self,...)
end,true)

--[[Koko.Utilities.Misc:FixUpValue(Tortoiseshell.Items.GetConfig,function(Old,Self,...)
...
Koko.Utilities.Misc:NewThreadLoop(0.025,function()
    CustomizeWeapon(
